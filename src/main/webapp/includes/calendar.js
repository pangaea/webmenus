...
RenderMonth.prototype.drawMonth = function( cellheight, width, month, year )
{
	...
	var now = new Date();
	if( month == null ) this.showingMonth = now.getMonth() + 1;
	...
	oMainCellC.className = "candendar_heading";
	...
	for( weekCount = 0; weekCount < 7; weekCount++ )
	{
		...
		for( weekDaysCount = 0; weekDaysCount < 7; weekDaysCount++ )
		{
			...
			else
			{
				...
				oDayInnerSpan.dayOfMonth = monthCount;
				oDayInnerSpan.parentCell = oDayCell;
				oDayInnerSpan.onclick = function(){this.parentCal.onSelectDay(this, this.dayOfMonth);};
/*
				...
				if( this.cellHeight != null )
					oDayInnerDiv.style.height = ((this.cellHeight)+"px");
...
RenderMonth.prototype.advanceMonth = function()
{
	...
	{
		nextMonth = parseInt(this.showingMonth) + 1;
		nextYear = this.showingYear;
	}
	else
	{
		nextMonth = 1;
		nextYear = parseInt(this.showingYear) + 1;
	}
...
RenderMonth.prototype.retractMonth = function(e)
{
	...
	{
		lastMonth = parseInt(this.showingMonth) - 1;
		lastYear = this.showingYear;
	}
	else
	{
		lastMonth = 12;
		lastYear = parseInt(this.showingYear) - 1;
	}
